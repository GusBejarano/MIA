...
/**
 * Clasifica un mensaje de conversacion libre en una de 4 intenciones, para
 * que MIA muestre la seleccion correspondiente (benefactores, menu de
 * categorias, o el carrusel de una categoria puntual) en vez de responder
 * con texto - el texto solo se usa cuando el mensaje de verdad no tiene
 * que ver con nada de esto (charla, agradecimientos, preguntas generales).
...
- CATEGORIA:<nombre exacto de la lista de abajo>: menciona una categoria
  ...
  CATEGORIA:Mascotas).
- NINGUNA: no tiene que ver con ver beneficios, categorias o programas
  (saludo, agradecimiento, pregunta general sobre como funciona MIA, etc.)

Programas/benefactores conocidos: ${benefactorNames.join(", ") || "(ninguno todavia)"}.

Responde EXACTAMENTE con una linea, uno de: NINGUNA / BENEFACTORES / CATEGORIAS / CATEGORIA:<nombre>`;
